...
class RealtimeWavDataset(speechset.WavDataset):
    ...
    def hook_reader(cls, reader: speechset.datasets.DataReader) -> int:
        ...
        elif isinstance(reader, PPIN):
            # wrapper
            def loader(path: str, sr: int):
                audio = PPIN.pcm_reader(path)
                return torch.tensor(audio), PPIN.SR, sr
            reader.load_audio = loader
        else:
